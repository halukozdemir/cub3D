...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/04/24 16:14:29 by halozdem          #+#    #+#             */
/*   Updated: 2025/04/24 16:14:29 by halozdem         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	set_wall_texture_x(t_ray *ray, t_image *texture)
{
...
static void	draw_wall_pixel(t_main *main, t_ray *ray, t_image *texture)
{
	int		tex_pixel;
	int		img_pixel;

	tex_pixel = ((((int)ray->textpos & (texture->height - 1))
				* texture->size_line) + ray->texture_x * (texture->bpp / 8))
		/ 4;
	img_pixel = (ray->draw_start * main->mlx.image.size_line + ray->x
			* (main->mlx.image.bpp / 8)) / 4;
	((unsigned int *)main->mlx.image.addr)
	[img_pixel] = ((unsigned int *)texture->addr)[tex_pixel];
}
...
void	display(t_main *main)
{
	...
	mlx_put_image_to_window(main->mlx.mlx, main->mlx.win, main->mlx.image.img,
		0, 0);
}
